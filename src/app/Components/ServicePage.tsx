...
export default function ServicesPage() {
  ...
  const navigationItems = [
    ...
    { name: t.nav.services, href: "/ServicePage", active: true }, // active here
...
<AnimatePresence>
  ...
  variants={{
    ...
    visible: {
      ...
      transition: {
        duration: 0.6,
        ease: "easeInOut" as const, // ✅ FIXED: replaced [0.4, 0, 0.2, 1]
      },
    },
    exit: {
      opacity: 0,
...
>
  ...
  >
    <div className="flex flex-col min-h-full pt-1 pb-6 relative">
      ...
      <motion.nav
        ...
        variants={{
          ...
          visible: {
            ...
            transition: {
              duration: 0.5,
              ease: "easeInOut" as const, // ✅ fix here too
            },
          },
        }}
        initial="hidden"
      ...
      {/* ============ Header (copied style from Contact, Services active) ============ */}
      <header className="bg-black/95 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2 flex items-center justify-between">
          <Link href="/">
            <Image src={logo2} alt="BIM" width={110} height={44} className="w-20 sm:w-24 md:w-28 lg:w-[110px] h-auto" />
          </Link>
          ...
          <nav className="hidden md:flex items-center space-x-6 lg:space-x-16 relative">
            {navigationItems.map((item, i) => (
              <Link
                key={i}
                href={item.href}
                className={`wave-link relative font-medium text-base ${item.active ? "text-red-500" : "text-white"}`}
              >
                {item.name}
        ...
        {/* wave underline styles (same as Contact) */}
        <style>{`
        ...
        <div className="mx-auto w-full max-w-[1200px] px-6 md:px-10 grid md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-14 mt-8 sm:mt-16 lg:mt-20 relative z-10">
          <div className="bg-black/40 border border-[#ff1f00] rounded-3xl p-4 hover:bg-black/60 transition-all">
            <div className="flex items-center text-[#ff1f00] space-x-2">
              ...
              <Link href="/WebsiteStrategy" className="font-medium text-white text-sm sm:text-base hover:text-[#ff1f00] inline-flex items-center gap-1 group">
                {t.services.knowMore}
      ...
      {/* ================== Footer (same as your Services footer) ================== */}
      <footer className="bg-black py-12 sm:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
            ...
            <div className="grid grid-cols-2 gap-8 mb-8 text-center">
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <Link href="/ServicePage" className="block text-white text-sm">{t.nav.services}</Link>
                  <Link href="/blog" className="block text-white text-sm">{t.nav.blog}</Link>
                </div>
              </div>
          ...
          <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
            ...
            <div className="text-left">
              ...
              <div className="space-y-2 sm:space-y-3 mt-4">
                ...
                <Link href="/ServicePage" className="block text-white text-sm sm:text-base">{t.nav.services}</Link>
                <Link href="/blog" className="block text-white text-sm sm:text-base">{t.nav.blog}</Link>
              </div>
            </div>
          ...
          <div className="hidden sm:block border-t border-gray-800 mt-8 sm:mt-12 pt-6 sm:pt-8 text-center text-white text-xs sm:text-sm">
            <a href="#" className="hover:text-white">{t.footer.privacyPolicy}</a> | <a href="#" className="hover:text-white">{t.footer.termsConditions}</a>
          </div>
        </div>
      </footer>
    </div>
